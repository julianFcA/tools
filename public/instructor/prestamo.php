...
$hora_finalizacion = date('Y-m-d 21:00', strtotime($fecha_adqui));
...
if (isset($_GET['documento'])) {
    $consulta_tipo = $conn->prepare("SELECT * FROM herramienta WHERE id_atrac = :id_atrac");
...
if (isset($_POST['limpiar_carrito'])) {
    ...
    header("Location: atracciones.php?limpiar_carrito=true");
    exit;
}
...
if (isset($_POST['usuario'])) {
    $consulta = $conn->prepare("SELECT documento FROM usuario WHERE documento = :documento");
    $consulta->bindParam(':documento', $_GET['documento']);
    $consulta->execute();
    $documento_cliente = $consulta->fetchColumn();
}

// Terminar la venta
if (isset($_POST['terminar_venta']) && isset($_SESSION['carrito']) && count($_SESSION['carrito']) > 0) {
    $documento_cliente_venta = isset($_POST['usuario']) ? $_POST['usuario'] : '';
    ...
    if ($documento = $documento_cliente_venta) {
        $sql_insert_compra->execute([$fecha_adqui, $hora_finalizacion, $documento, $id_esta_servi, $total_venta]);
    } else {
    ...
    header("Location: index.php?venta_completada=true");
    exit;
}
    ...
    <div class="row">
        <div class="col-md-8 offset-md-2">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" name="formreg" id="signup-form" class="signup-form" enctype="multipart/form-data" autocomplete="off" onsubmit="return validateForm()">
                        <div class="row">
...
<div class="row mt-4">
    <div class="col-12">
        <?php if (isset($_SESSION['carrito']) && count($_SESSION['carrito']) > 0) { ?>
            <?php
            $clientes = $conn->prepare("SELECT * FROM usuario WHERE id_rol = 2");
            $clientes->execute();
            $cliente = $clientes->fetch();
            ?>
            <form name="formreg" method="POST" class="form-container">
                <div class="form-group">
                    <label>Documento del Cliente</label>
                    <div class="form-group">
                        <!-- Group Type User -->
                        <label>Usuario:</label>
                        <select name="usuario" required id="" class='form-control'>
                            ...
                            ?>
                                <option value=<?php echo ($cliente['documento']) ?>><?php echo ($cliente['documento']) ?> </option>
                            <?php
